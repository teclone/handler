...
describe('DBChecker', function() {
    ...
    describe(`async #checkIfExists(required: boolean, field: string, value: DataValue,
        ...
        it(`should perform db check using mongoose executor by default if db model is nosql, setting error if field exists`, function() {
            return dbChecker.checkIfExists(true, 'email', 'someone@example.com', {
                if: 'exists',
                model: NoSqlUser,
                query: {
        ...
        it(`should perform db check using sequelize executor by default if db model is relational, setting error if field exists`, function() {
            return dbChecker.setDBModel(DB_MODELS.RELATIONAL).checkIfExists(true, 'email', 'someone@example.com', {
                if: 'exists',
                model: SqlUser,
                query: {
                    email: 'someone@example.com'
        ...
        it(`Nosql test: should set no error if field does not exist, resolving to true`, function() {
            return dbChecker.checkIfExists(true, 'email', 'someone1@example.com', {
                if: 'exists',
                model: NoSqlUser
            }, 0).then(succeeds => {
        ...
        it(`sql test: should set no error if field does not exist, resolving to true`, function() {
            return dbChecker.setDBModel(DB_MODELS.RELATIONAL).checkIfExists(true, 'email', 'someone1@example.com', {
                if: 'exists',
                model: SqlUser
            }, 0).then(succeeds => {
                expect(succeeds).toBeTruthy();
        ...
        it(`should perform no check if field is empty but not required, returning true`, function() {
            return dbChecker.checkIfExists(false, 'email', '', {
                if: 'exists',
                model: NoSqlUser
            }, 0).then(succeeds => {
                expect(succeeds).toBeTruthy();
    ...
    describe(`async #checkIfNotExists(required: boolean, field: string, value: DataValue,
        ...
        it(`should perform db check, setting error if field does not exist`, function() {
            return dbChecker.checkIfNotExists(true, 'email', 'someone1@example.com', {
                if: 'notExists',
                model: NoSqlUser,
                query: {
        ...
        it(`should set no error if field exists, resolving to true`, function() {
            return dbChecker.checkIfNotExists(true, 'email', 'someone@example.com', {
                if: 'notExists',
                model: NoSqlUser
            }, 0).then(succeeds => {
                expect(succeeds).toBeTruthy();
        ...
        it(`should perform no check if field is empty but not required, returning true`, function() {
            return dbChecker.checkIfNotExists(false, 'email', '', {
                if: 'notExists',
                model: NoSqlUser
            }, 0).then(succeeds => {
                expect(succeeds).toBeTruthy();
